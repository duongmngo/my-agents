...
class FileRepository(BaseRepository[File]):
    ...
    def get_workspace_files(self, workspace_id: str, folder_id: Optional[str] = None) -> List[File]:
    ...
    def get_file_by_storage_key(self, storage_key: str) -> Optional[File]:
    ...
    def get_file_by_hash(self, content_hash: str, workspace_id: str) -> Optional[File]:
        """Get file by content hash (for deduplication within workspace)"""
        with self._get_db() as db:
            return db.query(File).filter(
    ...
    def get_files_by_type(self, workspace_id: str, file_type: str, skip: int = 0, limit: int = 100) -> List[File]:
    ...
    def get_recent_files(self, workspace_id: str, days: int = 7, limit: int = 50) -> List[File]:
    ...
    def get_large_files(self, workspace_id: str, min_size_bytes: int, skip: int = 0, limit: int = 100) -> List[File]:
    ...
    def search_files(self, search_term: str, workspace_id: str, file_types: Optional[List[str]] = None, skip: int = 0, limit: int = 100) -> List[File]:
    ...
    def get_file_versions(self, parent_file_id: str) -> List[File]:
    ...
    def get_latest_version(self, parent_file_id: str) -> Optional[File]:
    ...
    def create_file_version(self, original_file_id: str, file_data: Dict[str, Any]) -> Optional[File]:
        """Create a new version of an existing file"""
        original_file = self.get_by_id(original_file_id)
        if not original_file:
            return None
        
        # Determine version number
        latest_version = self.get_latest_version(original_file_id)
    ...
    def get_storage_stats(self, workspace_id: str) -> Dict[str, Any]:
    ...
    def get_files_by_extension(self, workspace_id: str) -> Dict[str, int]:
    ...
    def move_file(self, file_id: str, new_folder_id: Optional[str]) -> bool:
    ...
    def toggle_pin(self, file_id: str) -> bool:
    ...
    def get_pinned_files(self, workspace_id: str) -> List[File]:
    ...
    def update_processing_status(self, file_id: str, status: str, error: Optional[str] = None) -> bool:
    ...
    def count_files(self, workspace_id: str, folder_id: Optional[str] = None) -> int:
        """Count files in a workspace using ORM count, optionally filtered by folder"""
        with self._get_db() as db:
            query = db.query(func.count(File.id)).filter(
                File.workspace_id == workspace_id,
                File.is_deleted == False
            )
